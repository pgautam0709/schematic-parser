...
Extracts word tokens with spatial coordinates per page using pdfplumber.
Falls back to PyMuPDF for image-heavy / low-text pages.
"""
...
def extract_page_words(pdf_path: str | Path) -> list[list[WordToken]]:
    """
    Extract word tokens per page.
    Returns a list (one per page) of WordToken lists.
...
def _fallback_pymupdf(pdf_path: str | Path, page_idx: int) -> list[WordToken]:
    """PyMuPDF fallback for pages with <10 pdfplumber tokens (scanned/image-heavy pages)."""
    import fitz  # PyMuPDF
